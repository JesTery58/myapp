...
@section('content')
   ...
   <?php
   use App\Category;
   function yolbul($catID,$lang) {

      $categorys = Category::where('id', $catID)->where('type', 'product')->orderBy('orders' , 'ASC')->get();
      foreach ($categorys as $cat){
         echo yolbul($cat->parent_id,$lang);
         echo '<li class="">'.(isset(unserialize($cat->name)[$lang]) ? unserialize($cat->name)[$lang] : "").'</li>';
      }
   } ?>
         ...
         <div class="row">
            <div class="col-md-8">
               <div class="title-base text-left">
                  <h1>{{ isset(unserialize($title)[$lang]) ? unserialize($title)[$lang] : "" }}</h1>
                  <p>
            ...
            <div class="col-md-4 cepcep">
               <ol class="breadcrumb b cepcep ">
               <li><a href="{{url('/')}}">{{ __('words.mainpage') }}</a></li>
               <li class="active">{{ __('words.products') }}</li>
               </ol>            
               </div>
         </div>
      </div>
                                 ...
                                 <div class="advs-box advs-box-multiple"  style="border: 1px solid #dcdcdc;border-radius: 5px;">
                                    <a  class="lightbox  " href="#column_dZjnQ" id="{{ $productCategorys->id }}" onclick="deneme(this.id)" style="min-height: 400px;
                                       ...
                                       <div style="min-height: 160px;padding: 2px">
                                          <img class="olcu2"  src="{{ asset(explode(',',$productCategorys->image)[0])}}"  style="width: 300px;height: 150px;object-fit: contain">
                                       </div>
                                    </a>
                                    <div class="advs-box-content" style="padding: 5px;background: #fbfbfb;">
                                       ...
                                       <p style="text-align: center;min-height: 50px;" >{{ unserialize( $productCategorys->caption)[$lang] ?? "" }}</p>
                                    </div>
                                 </div>
                              </div>
                           @endforeach
               ...
               <div class="col-md-6 col-sm-12 fade-left" data-anima="" style="position: relative; animation-duration: 500ms; transition-timing-function: ease; transition-delay: 0ms;" aid="0.9996004267456278">
                  <div class="row">
                     <div class="col-md-12">
                        ...
                        <!----
                        <div class="flexslider slider  visible-dir-nav  white">
                           <div class="flex-viewport" style="overflow: hidden; position: relative;">
                              <ul class="slides" style="width: 1000%; transition-duration: 0s; transform: translate3d(-1079px, 0px, 0px);">
                                 <li class="clone cep" aria-hidden="true" style="width: 359.976px; float: left; display: block;margin-left: -88px;">
                                 ...
                                 <li class="clone cep" aria-hidden="true" style="width: 359.976px; float: left; display: block;">
                                    <a class="img-box lightbox thumbnail inner" href="{{ asset('assets/images/bos.png') }}" data-lightbox-anima="show-scale">
                                            <span>
                                                <img alt="" src="{{ asset('assets/images/bos.png') }}" draggable="false">
                                            </span>
